...
function build_sea_and_the_land()
{
	echo "Extracting the coastline..."
        $OSMOSIS --read-pbf-fast file=$TEMP_LOC/sri-lanka-latest.osm.pbf --way-key-value keyValueList="natural.coastline" --tf reject-relations --used-node --write-pbf $TEMP_LOC/sri-lanka-latest-coastline.osm.pbf
	echo "Extracting the coastline...done."
	
	cd $IMG_LOC
	echo 'Sea and the land....'
	IMG_FILE_NAME="`shuf -i 10000000-99999999 -n 1`"
	$MKGMAP_JAVACMD $MKGMAP_JAVACMD_OPTIONS $MKGMAP -c $ARG_LOC/sea_land.args --dem=$HGT_LOC --description=Contours --mapname=$IMG_FILE_NAME --style-file=$STYLE_LOC --style=$STYLE_CONTOURS $TEMP_LOC/sri-lanka-latest-coastline.osm.pbf
...
function build_contour_lines()
{
	...
	IMG_FILE_NAME="`shuf -i 10000000-99999999 -n 1`"
	$MKGMAP_JAVACMD $MKGMAP_JAVACMD_OPTIONS $MKGMAP -c $ARG_LOC/elevation.args --description=Contours --mapname=$IMG_FILE_NAME --style-file=$STYLE_LOC --style=$STYLE_CONTOURS $MAP_ROOT/maps/srtm/*.osm.pbf
...
function build_roads_and_areas()
{
	...
	echo 'Splitting....'
	cd $SPLIT_LOC
	$MKGMAP_JAVACMD $MKGMAP_JAVACMD_OPTIONS $SPLITTER $TEMP_LOC/sri-lanka-latest-transport.osm.pbf
	echo 'Splitting done.'
	cd $MAP_ROOT
	...
	#$MKGMAP_JAVACMD $MKGMAP_JAVACMD_OPTIONS $MKGMAP -c $ARG_LOC/transport_osm.args --description=Transport --mapname=$IMG_FILE_NAME --style-file=$STYLE_LOC --style=$STYLE_TRANSPORT $SPLIT_LOC/*.osm.pbf
	#$MKGMAP_JAVACMD $MKGMAP_JAVACMD_OPTIONS $MKGMAP -c $ARG_LOC/combined.args --description=Transport --dem=$HGT_LOC --coastlinefile=$COASTLINE --mapname=$IMG_FILE_NAME --style-file=$STYLE_LOC --style=$STYLE_TRANSPORT $SPLIT_LOC/*.osm.pbf
